...
#   指定可能な名前: ACCOUNT_ID PROFILE REGION BUCKET PREFIX SWITCHBACK_SCRIPT ...
...
動作フラグ（パススルー）:
  --zip                  ディレクトリを個別ZIP化（--zip-directories）
  --dry-run              計画のみ（書き込みなし）
  --allow-overwrite      上書き許可
  --continue-on-error    失敗しても継続
  --verify-upload        アップロード後に検証
...
parse_args() {
  while [[ $# -gt 0 ]]; do
    case "$1" in
      ...
      --bucket)            _need "$1" "${2:-}"; BUCKET="$2"; shift 2 ;;
...
build_main_args() {
  ...
  [[ -n "$BUCKET" ]]     && MAIN_ARGS+=(--bucket "$BUCKET")
  ...
  if [[ -n "$RESULT_FILE" ]]; then
    MAIN_ARGS+=(--result-file "$(to_abs "$RESULT_FILE")" --result-format "$RESULT_FORMAT")
  fi
